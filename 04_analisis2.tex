...
Primero notemos que en Cálculo de una variable, la definición de función derivable en un punto (ver \ref{derivada}), es equivalente a pedir que
...
Un campo vectorial $ f : A \subseteq \RR^n \to \RR^m$ se dice que es una \textbf{función diferenciable en $ x_0 \in A^{\circ} $} \index{Diferenciabilidad} si existe una transformación lineal $ T : \RR^n \to \RR^m$ tal que
...
\begin{theorem}[Dif impl. deriv] \label{dif_impl_deriv}
...
\begin{theorem}[Dif impl. cont] \ref{dif_impl_cont}
...
\begin{observation}[Grad. normal Cjto nivel]
...
\begin{proposition}[Deriv. max, min, nula]
...
El siguiente teorema es una generalización directa de la regla de la cadena (ver \ref{regla_cadena}) de Cálculo en una variable.

\begin{theorem}[Regla de la cadena] \label{regla_cadena_matricial} \index{Diferenciabilidad!Regla de la cadena}
Sean
...
\begin{theorem}[Cauchy-Dini] \label{cauchy_dini} \index{Diferenciabilidad!Teo. Cauchy-Dini}
...
\begin{theorem}[Suficiente] \label{suficiente_potencial} \index{Integral de línea!Campo conservativo!Cond. suficiente}
...
En el caso que la superficie sea gráfica de campo escalar:
...
\begin{definition}[Irrot, solen, armónico]
