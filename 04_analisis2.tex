...
donde $a_n, \ldots, a_0 \in \mathbb{N}$
...
Dada una ecuación en la que interviene una variable independiente, una variable dependiente, y $n$ constantes arbitrarias $ c_1, \ldots, c_n \in \mathbb{R}$, llamamos \textbf{familia de curvas} (de orden $n$) a las curvas que se obtienen de asignarle valores a las constantes arbitrarias.
...
Llamamos \textbf{espacio euclídeo}, a un espacio vectorial sobre $\mathbb{R}$ con un producto interno. 
\end{definition}

\begin{definition} \label{producto escalar}
Definimos el \textbf{producto interno usual de $\mathbb{R}^n$} (o producto escalar) de la siguiente manera: dados $v,w \in \mathbb{R}^n$, su producto interno usual es

$$ \vec{v} \cdot \vec{w} = v_1w_1 + v_2w_2 + \ldots + v_nw_n $$
\end{definition}

El espacio euclídeo en el que vamos a trabajar en esta materia es $ \mathbb{R}^n$ con el producto interno usual.  De ahora en más cada vez que se mencione $\mathbb{R}^n$ lo vamos a pensar como un espacio euclídeo.
...
Dado $v \in \mathbb{R}^n$, definimos su \textbf{norma} como

$$ ||\vec{v}|| = \sqrt{\vec{v} \cdot \vec{v}} = \sqrt{v_1^2 + v_2^2 + \ldots v_n^2} $$

y dados $v,w \in \mathbb{R}^n$, definimos su \textbf{distancia} como
...
\section{Clasificación topológica de puntos de $\mathbb{R}^n$}

\begin{definition}

Sea $A \subseteq \mathbb{R}^n$, y $x \in \mathbb{R}^n$, entonces decimos que $x$ respecto de $A$ es 

\begin{itemize}

\item \textbf{Punto interior}: Si existe $E(x,\delta) \subseteq A$.

\item \textbf{Punto exterior}: Si existe $E(x,\delta) \subseteq \mathbb{R}^n - A$.  Equivaléntemente $E(x,\delta) \cap A = \emptyset$.

\item \textbf{Punto frontera}: Si no es punto interior ni exterior.  Es decir que para todo $ \delta > 0$ se tiene $E(x,\delta) \cap A \neq \emptyset$ y $E(x,\delta) \cap (\mathbb{R}^n - A) \neq \emptyset$
...
Sea $A \subseteq \mathbb{R}^n$.  Entonces definimos
...
Dado $A \subseteq \mathbb{R}^n$, se cumple que $ A^{\circ} \subseteq A \subseteq \overline{A}$.

\end{observation}

\section{Clasificación topológica de subconjuntos de $\mathbb{R}^n$}

\begin{definition}
Sea $A \subseteq \mathbb{R}^n$, decimos que $A$ es un conjunto
...
\begin{observation}
Un conjunto puede no ser ni abierto ni cerrado. Por ejemplo $ [0,1) \subset \mathbb{R}$.

Además un conjunto puede ser abierto y cerrado a la vez. Por ejemplo $ \emptyset$ y $ \mathbb{R}^n$.
\end{observation}

\section{Conjunto acotado}

\begin{definition}
Un conjunto $A \subseteq \mathbb{R}^n$ se dice que es un \textbf{conjunto acotado} si el mismo está contenido en algún entorno del origen, es decir si $ A \subseteq E(0, r)$ para algún $r > 0$.
\end{definition}
...
\begin{definition}
Una \textbf{escisión} de un conjunto $X \subseteq \mathbb{R}^n$ son dos conjuntos disjuntos abiertos $A,B$ tales que $ X \subseteq A \cup B$.  Decimos que la misma es no trivial si $A \cap X \neq \emptyset$ y $B \cap X \neq \emptyset$.

Un conjunto $ X \subseteq \mathbb{R}^n$ es un \textbf{conjunto conexo} si no admite escisiones no triviales.  
...
\item $ B = S^1 = \{ (x,y) \in \mathbb{R}^2 : x^2 + y^2 = 1\}$

\item $ C = D^2(4,4) = \{ (x,y) \in \mathbb{R}^2 : (x-4)^2 + (y-4)^2 \leq 1 \}$ 
\end{itemize}
...
\begin{definition}
Si $a,b \in \mathbb{R}^n$, un \textbf{camino} de $a$ a $b$ es una función $ \alpha : [0,1] \to \mathbb{R}^n$ contínua, tal que $\alpha(0) = a$ y $\alpha(1) = b$
\end{definition}

\begin{example}
Si $a, b \in \mathbb{R}^n$ el \textbf{camino recto} entre $a$ y $b$ es la función $\alpha : [0,1] \to \mathbb{R}^n$ tal que
...
\begin{definition}
Un conjunto $A \subseteq \mathbb{R}^n$ se dice que es un \textbf{conjunto arco-conexo} si para todos los $a,b \in A$ existe un camino $\alpha$ que une $a$ con $b$ dentro del conjunto, es decir tal que $\alpha([0,1]) \subseteq A$.
\end{definition}
...
\begin{definition}
Si $\alpha$ es un camino de $a$ a $b$, y $\beta$ es un camino de $b$ a $c$, definimos la \textbf{yuxtaposición} de $\alpha$ y $\beta$ como el camino $ \alpha \wedge \beta : [0,1] \to \mathbb{R}^n$ de $a$ a $c$ definido por 
...
\begin{definition}
Una \textbf{poligonal} $\pi$ es la yuxtaposición de una cantidad $k \in \mathbb{N}$ finita de caminos rectos $\pi_i : [x_{i-1}, x_i] \to \mathbb{R}^n$ con $1 \leq i \leq k$. 
\end{definition}

\begin{definition}
Un conjunto $A \subseteq \mathbb{R}^n$ se dice que es un \textbf{conjunto conexo por poligonales} si para todo $a, b \in A$ existe una poligonal $\pi$ que comienza en $a$, termina en $b$, y está contenida en $A$.
\end{definition}
...
\begin{definition}
Un conjunto $A \subseteq \mathbb{R}^n$ se dice que es un \textbf{conjunto convexo} si para todo $a, b \in A$ el camino recto que los une no se sale del conjunto, es decir $[a,b] \subseteq A$.
\end{definition}
...
Esta es la noción más compleja de conexidad que vemos.  Intuitivamente, un conjunto es símplemente conexo si toda curva cerrada simple (curva de Jordan) se puede "deformar contínuamente" hasta llegar a un punto.  En $ \mathbb{R}^2$ esto sería equivalente a decir que el conjunto no tiene agüjeros.  

Más formalmente, si denotamos $S^1$ al círculo unitario de $\mathbb{R}^2$ y $D^2$ al disco unitario de $\mathbb{R}^2$, es decir
...
\begin{definition}
Sea $f$ una función de la forma $ f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$.  
...
Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$ un campo escalar, y sea $k \in \mathbb{R}$
...
Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$ un campo escalar.
...
Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}^m$, $ a \in A'$, y $L \in \mathbb{R}^n$.  Se dice que existe el límite de $f$ cuando $x$ tiende a $a$ es $L$ si se cumple cualquiera de las siguientes afirmaciones equivalentes
...
Sea $ f: A \subset \mathbb{R}^n \to \mathbb{R}^m$, y sea $a \in A$.  Se dice que $f$ es \textbf{contínua} en $a$ si se cumple cualquiera de las siguientes afirmaciones equivalentes
...
Si $f : A \subseteq \mathbb{R}^n \to B \subseteq \mathbb{R}^m$ es contínua en $a$ y $ g : B \subseteq \mathbb{R}^m \to \mathbb{R}^p$ es contínua en $b = f(a)$, y si llamo $h = g \circ f$, entonces $h$ es contínua en $a$.
...
\begin{observation}
Si $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ tiene funciones coordenadas $f = (f_1, f_2, \ldots, f_m)$ entonces $f$ es contínua en $a$ si y solo sí cada componente $f_i$ es contínua en $a$ para todo $1 \leq i \leq m$.
\end{observation}
\begin{proof}[Demostración (Optativa)]
$\Rightarrow$) Suponemos $f$ contínua en $a$. Considero la proyección $P_j : \mathbb{R}^m \to \mathbb{R}$ tal que $P_j(x_1, \ldots, x_m) = x_j$.  Es Lipschitziana, pues $|x_j - y_j| \leq 1 \cdot || \vec{x} - \vec{y} ||$.
...
\begin{observation}
Supongamos que tenemos $f: A \subseteq \mathbb{R}^n \to B \subseteq \mathbb{R}^m$, $g : B \subseteq \mathbb{R}^m \to \mathbb{R}^p$ y $h = g \circ f$, tales que $\lim_{x \to a} f(x) = L_f$ y $\lim_{y \to L_f} g(y) = L_g$.  Todo ello no implica que $\lim_{x \to a} h(x) = L_g$
\end{observation}
\begin{proof}
...
Dados $f: A \subseteq \mathbb{R}^n \to B \subseteq \mathbb{R}^m$, $g : B \subseteq \mathbb{R}^m \to \mathbb{R}^p$, y $h = g \circ f$, tales $\lim_{x \to a} f(x) = L$, y que $g$ es contínua en $L$, con $c = g(L)$ entonces $\lim_{x \to a} h(x) = c$
\end{observation}
\begin{proof}
...
Dados $f: A \subseteq \mathbb{R}^n \to B \subseteq \mathbb{R}^m$, $g : B \subseteq \mathbb{R}^m \to \mathbb{R}^p$, y $h = g \circ f$, tales $\lim_{x \to a} f(x) = L_f$, y $\lim_{y \to L_f} g(y) = L_g$, y si además para todo $\epsilon > 0$ existe $\delta > 0$ tal que si $x \in A$, $0 < ||x-a|| < \delta$ luego $0 < || f(x) - L_f|| < \epsilon$, pues entonces $\lim_{x \to a} h(x) = L_g$
\end{observation}
\begin{proof}
...
\begin{theorem}
Sea $f:A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ con funciones coordenadas $ f = (f_1, \ldots, f_m)$ y $a \in A'$, entonces el límite $ \lim_{x \to a} f(x) = L$ existe si y sólo si existen todos los límites $ \lim_{x \to a} f_i(x) = L_i$ con $ 1 \leq i \leq m$, y en dicho caso se tiene $ L = (L_1, \ldots, L_m)$
\end{theorem}
\begin{proof}
...
Supongamos que las funciones $f, g : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ tales que existen $\lim_{x \to a} f(x) = L_1$ y $\lim_{x \to a} g(x) = L_2$.
...
Ejemplo: $x^2 + 3xy + z^3$ es una función contínua en $\mathbb{R}^3$

\item La función exponencial $f(x) = e^x$ es contínua en $\mathbb{R}$

\item Las funciones trigonométricas $\sin(x)$ y $\cos(x)$ son contínuas en $\mathbb{R}$
\end{itemize}

Supongamos que las funciones $f, g : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ son contínuas en $a \in A$. 
...
\begin{definition}
Sea $A \subseteq \mathbb{R}^n$ y sea $a \in A'$.  Un \textbf{camino que pasa por $a$} (o que se aproxima a $a$) es un subconjunto $B \subseteq A$ tal que $a \in B'$ (es decir tal que el punto $a$ es punto de acumulación también de $B$).

Un camino $B$ que pasa por $a$ se dice que es una \textbf{vecindad de $a$} si existe $E(a,\delta) = E$ tal que $E \cap A \subseteq B$.
\end{definition}

\begin{example}
Por ejemplo, dado $A \subseteq \mathbb{R}^n$ y $a \in A'$ y $E = (a, \delta) = E$.  Entonces $B = E \cap A$ es una vecindad de $a$.
\end{example}
...
Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}^m$, $a \in A'$, $B$ una vecindad de $a$, y $g = f|_B$.  Entonces $\lim_{x \to a} f(x) = \lim_{x \to a} g(x)$.  Es decir el límite de $f$ existe sii existe el límite de $g$, y en ese caso son iguales.
\end{observation}
...
\begin{proposition}
Sea $ f : A \in \mathbb{R}^n \to \mathbb{R}$, y $B \subseteq A$ un camino que pasa por $a$ y $g = f|_B$.  Si existe el límite $ \lim_{x \to a} f(x) = L$, entonces también existe el límite $ \lim_{x \to a} g(x) = L$ y son iguales.
\end{proposition}
...
\begin{proposition}
Sean $f,g,h:A \subseteq \mathbb{R}^n \to \mathbb{R}$, y $a \in A'$.
...
Dada la función vectorial $ f: A \subset \mathbb{R} \to \mathbb{R}^n$ y $ t_0 \in A^{\circ}$, entonces la \textbf{derivada de $f$ en $t_0$} se define como
...
Si $f : [a,b] \to \mathbb{R}^n$ es la parametrización de una curva $C$, y si además $f$ es derivable en $t_0$, entonces $f'(t_0)$ es un vector tangente a dicha curva en el punto $f(t_0)$.  
...
Sea $f : [a,b] \to \mathbb{R}^n$ parametrización de una curva $C$, y derivable en $t_0$.  Entonces la \textbf{recta tangente a $C$ en $x_0 = f(t_0)$} es de ecuación paramétrica
...
Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ y $x_0 \in A^{\circ}$, y sea $v \in \mathbb{R}^n$.  Entonces definimos la \textbf{derivada de $f$ en $x_0$ respecto al vector $v$} como
...
Si además $v$ es un versor canónico de $\mathbb{R}^n$, es decir si 
...
Dada $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$, 

Decimos que \textbf{$f$ es derivable respecto a $v \in \mathbb{R}^n$}, sin aclarar el punto, si lo es para todo $x \in \mathbb{R}^n$.

Decimos que \textbf{$f$ es derivable en $x_0 \in A^{\circ}$}, sin aclarar el vector, si lo es respecto a todo $v \in \mathbb{R}^n$.

Decimos que \textbf{$f$ es derivable}, sin aclarar ni el punto ni el vector, si lo es para todo $x \in A^{\circ}$ y respecto a todo $v \in \mathbb{R}^n$.
...
\begin{theorem}
Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ con $x_0 \in A^{\circ}$ y $v \in \mathbb{R}^n$, tal que $f = (f_1, \ldots, f_m)$.  Entonces $f$ es derivable en $x_0$ respecto a $v$ si y sólo sí cada función coordenada es derivable en $x_0$ respecto a $v$, y en dicho caso se tiene $f'_v(x_0) = (f'_{1v}(x_0), f'_{2v}(x_0), \ldots, f'_{mv}(x_0))$.
 \end{theorem}
...
Si existe la derivada direccional $ f'(x_0, v)$ y $ k \in \mathbb{R}$, $ k \neq 0$, entonces 
...
\begin{definition}
Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}^m$.  Supongamos esta función es derivable respecto a $x_i$, entonces podemos definir la función derivada parcial $f'_{x_i} : A \to \mathbb{R}^n$.  Supongamos ahora que esta nueva función es derivable  respecto a $x_j$.  Podemos definir entonces la función derivada segunda parcial

$$ f''_{x_i x_j} : A \to \mathbb{R}^m $$
...
$$ f^{(k)}_{x_{i_1} x_{i_2} \ldots x_{i_k}} : A \to \mathbb{R}^m $$
...
\begin{theorem} \label{schwarz} 

Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}$, si existen $ f''_{x_i x_j}, f''_{x_j x_i}$ y son contínuas en un entorno del punto $ x_0 \in A^{\circ}$ entonces 
...
\begin{definition}
Si $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ con $A$ abierto, es tal que posee todas sus derivadas parciales y son contínuas en $ A$, entonces decimos que $ f$ es de clase $ C^1$ y lo denotamos 
...
\begin{observation}
Si $f : A \subset \mathbb{R}^n \to \mathbb{R}$, $f \in C^2$, para cualquier $x \in A$ se cumplen las hipótesis del teorema \ref{schwarz} (de Schwarz), es decir  $ f''_{x_i x_j}(x) = f''_{x_j x_i}(x)$.
...
Un subconjunto $C \subseteq \mathbb{R}^n$ decimos que es una \textbf{curva} si existe una función vectorial contínua $g: [0,1] \to \mathbb{R}^n$, tal que $ g([0,1]) = C$.  A la función $ g$ se la llama parametrización de la curva.
...
Un subconjunto $ \Sigma$ de $ \mathbb{R}^3$ decimos que es una \textbf{superficie} si existe un campo vectorial contínuo $ g: A \subset \mathbb{R}^2 \to \mathbb{R}^3$, con $A$ conexo, tal que $ g(A) = S$.  A la función $g$ se le dice la parametrización de la superficie $\Sigma$.
...
\begin{eqnarray*} g : \mathbb{R} &\to& \mathbb{R}^2 \\
...
Una función escalar $ f : A \subset \mathbb{R} \to \mathbb{R}$ es derivable en $ x_0 \in A^{\circ}$ si existe el siguiente límite
...
O sea es derivable sii existe una función escalar $ \mu : A \to \mathbb{R}$ tal que
...
Por otro lado, la expresión $ f'(x_0) h$ es una transformación lineal $T : \mathbb{R} \to \mathbb{R}$, $h \to T(h)$.

Por lo tanto podemos decir que $f$ es derivable sii existe una transformación lineal $T : \mathbb{R} \to \mathbb{R}$ y un campo escalar $ \mu : A \to \mathbb{R}$ tales que
...
\begin{definition}
Un campo vectorial $ f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ se dice que es una \textbf{función diferenciable en $ x_0 \in A^{\circ} $} si existe una trasnformación lineal $ T : \mathbb{R}^n \to \mathbb{R}^m$ y un campo vectorial $ \mu : B(x_0, \delta) \cap A \to \mathbb{R}^m$ tales que
...
Sea el campo vectorial $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ diferenciable en $ x_0 $, entonces $f$ es derivable en $x_0$, y además las derivadas valen 
...
\begin{proof}
Como $f$ es diferenciable en $x_0$ existen una transformación lineal $T : \mathbb{R}^n \to \mathbb{R}^m$ y un campo vectorial $ \mu : \mathbb{R}^n \to \mathbb{R}^m$ tales que
...
\begin{corollary}
Sea un campo vectorial $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ diferenciable en $x_0 \in A^{\circ}$, y sea $T$ una transformación lineal que cumple lo pedido, entonces dicha transformación lineal es única, y además 
...
Para ver que la transformación lineal es única, como $\{e_i\}_{1 \leq i \leq n}$ es una base de $\mathbb{R}^n$, cualquier vector $x \in \mathbb{R}^n$ lo puedo escribir en forma única como combinación lineal $x = \sum_{i=1}^n \lambda_i e_i$, luego si $T : \mathbb{R}^n \to \mathbb{R}^m$ es una transformación lineal debe cumplir $T(x) = \sum_{i=1}^n \lambda_i T(e_i)$, y por lo visto recién debe cumplir  $T(x) = \sum_{i=1}^n \lambda_i f'(x_0, e_i)$, con lo cual quedó complétamente determinada, es decir la transformación lineal es única.

\end{proof}

\begin{definition}

Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$  diferenciable en $x_0 \in A^{\circ}$, de la forma $ f = (f_1, f_2, \ldots, f_m)$, y sea $T : \mathbb{R}^n \to \mathbb{R}^m$ el diferencial de $f$ en $x_0$.

Sea $[T]$ la matriz asociada a la transformación lineal $T$ respecto a las bases canónicas de $\mathbb{R}^n$ y $\mathbb{R}^m$.  La misma se expresa poniendo en sus columnas los transformados de la base canónica, es decir las derivadas parciales de $f$ en $x_0$, es decir que
...
Al diferencial $T$ de $f$ en $x_0$ lo vamos a denotar también $df$.  Y a la matriz $[T]$ asociada al diferencial la vamos a llamar la \textbf{matriz jacobiana de $f$ en $x_0$}, y la denotamos $Df$.
...
\begin{theorem}
Sea el campo vectorial $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ diferenciable en $ x_0 \in A^{\circ}$, entonces $f$ es contínuo en $x_0$.
\end{theorem}

\begin{proof}
Como $ f$ es diferenciable en $x_0$ existen una (única) transformación lineal $ T : \mathbb{R}^n \to \mathbb{R}^m$ y un campo vectorial $ \mu : A \to \mathbb{R}^m$ tales que
...
\begin{definition}
Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$, y $x_0 \in A^{\circ}$, si existen las derivadas parciales de $f$ en $x_0$, definimos el \textbf{vector gradiente $\nabla f(x_0)$ de $f$ en $x_0$} como el vector cuyas coordenadas son las derivadas parciales de $f$ en $x_0$, es decir como
...
Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$, diferenciable en $x_0 \in A^{\circ}$, y sea $v \in \mathbb{R}^n$. El teorema \ref{difderiv} nos dice en este caso que
...
\begin{observation}
Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}$ diferenciable en $x_0 \in A^{\circ}$, y sea $ k \in \mathbb{R}$, entonces $ \nabla f(x_0)$ es perpendicular al conjunto de nivel $k$ de $f$.
\end{observation}
...
Sea $ g:[a,b] \to \mathbb{R}^n$ la parametrización de una curva incluída en el conjunto de nivel, es decir que $g([a,b]) \subseteq C_k(f)$, tal que $g(t_0) = x_0$ y $g'(t_0) \neq 0$.
...
\begin{proposition}
Sea $ f : A \subset \mathbb{R}^n \to \mathbb{R}$ es diferenciable en $ x_0 $ y $\nabla f(x_0) \neq 0$.  Entonces:
...
\begin{theorem}
Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ con $A$ abierto.
...
Por ejemplo, la siguiente función es diferenciable en $\mathbb{R}$, pero su derivada $f'(x)$ no es contínua en el origen.
...
\begin{definition}
Sean $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ y $ g : B \subset \mathbb{R}^m \to \mathbb{R}^p$ tales que $f(A) \subseteq B$.

Entonces existe una única la función $ h : A \to \mathbb{R}^p$ tal que $h(x) = g(f(x))$.  
...
Al mismo tiempo es más particular que aquella definición, pues la primera se refiere funciones entre conjuntos en general, mientras que la segunda es entre subconjuntos de $\mathbb{R}^n$.

\section{Regla de la Cadena}

\begin{theorem} \label{regla_cadena}
Sean $ f : A \subset \mathbb{R}^n \to \mathbb{R}^m$ diferenciable en $x_0 \in A^{\circ}$, y $ g : B \subset \mathbb{R}^m \to \mathbb{R}^p$ diferenciable en $y_0 = f(x_0) \in B^{\circ}$, tales que $f(A) \subseteq B$

Entonces la función compuesta $h : A \to \mathbb{R}^p$ es diferenciable en $x_0$, y además el diferencial de $h$ en $x_0$ es igual a la composición de los diferenciales de $f$ en $x_0$ con el de $g$ en $y_0$, de la siguiente manera
...
\begin{definition}
Dada una función $F : A \subseteq \mathbb{R}^{n+1} \to \mathbb{R}$, decimos que la ecuación $F(x_1, x_2, \ldots, x_n, y) = 0$ \textbf{define implícitamente a $y$ como función de $x_1, \ldots, x_n$ en $B$} si existe una función $ f : B \subseteq \mathbb{R}^n \to \mathbb{R}$ tal que $F(x_1, \ldots, x_n, f(x_1, \ldots, x_n)) = 0$, para $B \subseteq A_y$ donde $A_y$ es la proyección de $A$ sobre $x_1, \ldots, x_n$
\end{definition}

Ejemplo: $F: \mathbb{R}^3 \to \mathbb{R}$, $F(x,y,z) = x^2 + y^2 - z$.  La ecuación correspondiente es $x^2 + y^2 - z = 0$. Como existe $f : \mathbb{R}^2 \to \mathbb{R}$, $f(x,y) = x^2 + y^2$ tal que $F(x,y,f(x,y)) = 0$, la ecuación define implícitamente a $z$ como función de $x,y$.

Otro ejemplo: $F: \mathbb{R}^3 \to \mathbb{R}$, $F(x,y,z) = x^2 + y^2 + z^2 - 1$.  La ecuación correspondiente es $x^2 + y^2 + z^2 = 1$. 

Sea $D_2 = \{ (x,y) \in \mathbb{R}^2 : x^2 + y^2 \leq 1 \}$. Como existe $f : D_2 \subseteq \mathbb{R}^2 \to \mathbb{R}$, $f(x,y) = \sqrt{1 - x^2 + y^2}$ tal que $F(x,y,f(x,y)) = 0$, la ecuación define implícitamente a $z$ como función de $x,y$.

Notar que globálmente la relación entre $x,y$ y $z$ no es funcional, pues por ejemplo tanto el $(0,0,1)$ como el $(0,0,-1)$ satisfacen la ecuación, pero $f(0,0)$ debe tener una única imagen.  En particular, la función definida implícitamente no tiene porqué ser única, otra función definida implícitamente por la misma ecuación es $g : D_2 \to \mathbb{R}^2$, $g(x,y) = - \sqrt{1 - x^2 - y^2}$

\section{Teorema de Cauchy-Dini}

\begin{theorem}{Teorema de Cauchy-Dini} \label{cauchy_dini}
Sea $F : A \subseteq \mathbb{R}^{n+1} \to \mathbb{R}$ con $A$ abierto y $F \in C^1$, y sea la ecuación $F(x_1, \ldots, x_n, y) = 0$.
...
Este teorema nos garantiza bajo ciertas condiciones la existencia de una función definida implícitamente, es decir que exista $ f : A_y \cap E(P_y, \delta) \subset \mathbb{R}^n \to \mathbb{R}$ tal que $F(x_1, \ldots, x_n, f(x_1, \ldots, x_n)) = 0$
...
Sea $f: A \subseteq \mathbb{R}^2 \to \mathbb{R}$, $f \in C^2$, $(x_0, y_0) \in A$.
...
El polinomio de Taylor de primer grado de $f$ en $(x_0, y_0)$ es

\begin{eqnarray*} T_1(u,v) &=& f(x_0,y_0) + df_{(x_0, y_0)}(u,v) \\ 
&=& f(x_0,y_0) + f'_x(x_0,y_0)u + f'_y(x_0,y_0)v \end{eqnarray*}

Y el polinomio de Taylor de segundo grado de $f$ en $(x_0, y_0)$ es

\begin{eqnarray*} T_2(u,v) &=& f(x_0,y_0) + df_{(x_0,y_0)}(u,v) + \frac{1}{2!} d^2f_{(x_0,y_0)}(u,v) \\ 
&=& f(x_0,y_0) + f'_x(x_0,y_0)u + f'_y(x_0,y_0)v + \\
&+& \frac{1}{2!} \left[ f''_{xx}(x_0,y_0)u^2 + 2 f''_{xy}(x_0,y_0)uv + f''_{yy}(x_0,y_0)v^2 \right] \end{eqnarray*}

Más generalmente, si $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$, $f \in C^k$, $x_0 \in A$, y llamando $u = x - x_0$, el polinomio de Taylor de $f$ de grado $k$ en $x_0$ es

$$ T_k(u) = f(x_0) + df_{x_0}(u) + \frac{1}{2!} d^2f_{x_0}(u) + \ldots + \frac{1}{k!}d^kf_{x_0}(u) $$

\end{definition}

El polinomio de Taylor sirve para aproximar la función en un entorno del punto donde fué calculada.  Es decir si $f \in C^k$ y $T_k(u)$ es el polinomio de grado $k$ de $f$ en $x_0$, entonces

$$ f(x) \approx T_k(x-x_0)$$
...
Sabemos que $\mathbb{R}$ es un conjunto totalmente ordenado (ver definición \ref{orden_total}), y por lo tanto cualquier subconjunto $B \subseteq \mathbb{R}$ también hereda un orden total.

Tiene sentido entonces para $B \subseteq \mathbb{R}$ preguntar si tiene un máximo o un mínimo.

Por ejemplo, si $A = (0,1]$, entonces $A$ tiene un máximo y es el elemento $1 \in A$, pero no tiene ningún mínimo.

\begin{definition}
Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$ un campo escalar, y sea $Y = Im(f) \subseteq \mathbb{R}$ el conjunto imagen de $f$.  
...
Sea $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$ un campo escalar, $x_0 \in A$, $E = E(x_0, \delta)$ un entorno de $x_0$ de radio $\delta > 0$, y $g = f|_{E}$, es decir la función $f$ restringida al entorno $E$, y sea $Y_E = Im(g)$ la imagen de la función así restringida.
...
Sea el campo escalar $ f : A \subset \mathbb{R}^n \to \mathbb{R}$, y $ x_0 \in A$, entonces:
...
Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}$.  Un \textbf{punto crítico de $f$} es un elemento $x_0 \in A$ tal que o bien $f$ no es diferenciable en $x_0$, o bien es diferenciable en $x_0$ pero $ \nabla f(x_0) = 0$.
...
\begin{theorem}{Criterio de la derivada primera} \label{criterio_derivada_primera}

Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}$, $ f \in C^1$, entonces una condición necesaria para que $f(x_0)$ sea extremo, es que $x_0$ sea punto crítico.
\end{theorem}

Es decir que el criterio de la derivada primera nos dice que si $x_0 \in A$ es punto crítico, entonces o bien $f(x_0)$ es extremo, o bien $(x_0, f(x_0))$ es punto silla.

\begin{proof}
Sea $g(t) = x_0 + t v$ para un versor $v \in \mathbb{R}^n$, y considero la composición 
...
En particular tomando $v$ un versor \emph{canónico} de $\mathbb{R}^n$ vemos que todas las derivadas parciales son nulas, y por lo tanto $ \nabla f(x_0) = 0$ como queríamos probar.
...
\begin{definition}
Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}$, $f \in C^2$.
...
Sea $M \in \mathbb{R}^{n \times n}$ una matriz cuadrada y simétrica.

Entonces $M$ tiene autovalores $\lambda_1, \lambda_2, \ldots, \lambda_n \in \mathbb{R}$, es decir tiene todos sus autovalores y son reales.
\end{theorem}

\begin{definition} \label{definida_positiva}

Sea $M \in \mathbb{R}^{n \times n}$ una matriz cuadrada y simétrica con coeficientes reales.
...
\begin{theorem}{Criterio de la derivada segunda} \label{criterio_derivada_segunda}

Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}$, $ f \in C^2$ y $x_0 \in A$ punto crítico.  Entonces si la matriz hessiana de $f$ en $x_0$ es \emph{definida positiva} (Ver \ref{definida_positiva}), la función presenta un mínimo relativo $f(x_0)$, y si es \emph{definida negativa}, la función presenta un máximo relativo $f(x_0)$.
...
Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}$, $ f \in C^2$ y $x_0 \in A$ punto crítico.
...
\begin{example}
Veamos el caso particular $n=3$.  Sea $f : A \subseteq \mathbb{R}^3 \to \mathbb{R}$, $f \in C^2$ y $x_0 \in A$ punto crítico de $f$.  Queremos saber si $f(x_0)$ es extremo relativo. Sea $Hf(x_0)$ la matriz hessiana de $f$ en $x_0$
...
\begin{example}
Ahora veamos el caso particular $n=2$.  Sea $f : A \subseteq \mathbb{R}^2 \to \mathbb{R}$, $f \in C^2$ y $x_0 \in A$ punto crítico de $f$.  Queremos saber si $f(x_0)$ es extremo relativo. Sea $Hf(x_0)$ la matriz hessiana de $f$ en $x_0$
...
Sea $[a,b] \in \mathbb{R}$ un intervalo compacto, y sea $ P = \{a = x_0 < x_1 < \ldots < x_n = b\}$ una partición de $ [a,b]$.  Sea $f$ una función acotada en $ [a,b]$.  
...
\begin{definition}
Una curva $C \in \mathbb{R}^n$ decimos que es \textbf{regular a trozos} si puede escribirse como $\displaystyle C = \cup_{i=1}^k C_i$ donde cada $C_i$ es una curva regular, y el extremo final de una coincide con el inicial de la siguiente.

Es decir si cada curva $C_i$ está parametrizada por $g_i : [a_i, b_i] \to \mathbb{R}^n$, entonces $g_i(b_i) = g_{i+1}(a_{i+1})$

Una curva $C$ es \textbf{simple} si admite una parametrización inyectiva.

Una curva $C$ es \textbf{cerrada} si admite una parametrización $g:[a,b] \to \mathbb{R}^n$ tal que $g(a) = g(b)$

Una curva $C$ se dice cerrada simple, o \textbf{curva de Jordan}, si admite una parametrización $ g:[a,b] \to \mathbb{R}^n$ tal que es inyectiva en $ [a,b)$ y $ g(a) = g(b)$
\end{definition}

\section{Integral de línea}

\begin{definition}
Dada una curva $C \in \mathbb{R}^n$ con parametrización $ g:[a,b] \to \mathbb{R}^n$, y dado un campo escalar $f : A \subseteq \mathbb{R}^n \to \mathbb{R}$ contínuo, tal que $C \in A^{\circ}$, definimos el diferencial de curva escalar como
...
Dada la misma curva $C$ con misma parametrización $g$, y dado el campo vectorial $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^n$ contínuo, tal que $C \in A^{\circ}$, definimos el diferencial de curva vectorial como
...
Dado un campo vectorial $ f: A \subseteq \mathbb{R}^n \to \mathbb{R}^n$, con $A$ abierto y $f \in C^1$, podemos preguntarnos si en realidad se trata del gradiente de un campo escalar $\phi : A \subseteq \mathbb{R}^n \to \mathbb{R}$, $\phi \in C^2$, tal que $ \nabla \phi = f$.  Esto motiva la siguiente

\begin{definition}
Dado $ f: A \subseteq \mathbb{R}^n \to \mathbb{R}^n$, con $A$ abierto y $f \in C^1$.  

Si existe $\phi : A \subseteq \mathbb{R}^n \to \mathbb{R}$, $\phi \in C^2$, tal que 
...
\begin{theorem}
Sea el campo vectorial conservativo $f : A \subset \mathbb{R}^n \to \mathbb{R}^n$, y sea $ \phi : A \subset \mathbb{R}^n \to \mathbb{R}$ su función potencial. 

Sea $ C \subset A^{\circ}$ una curva regular con parametrización $ g:[a,b] \to \mathbb{R}^n$.
...
\begin{theorem} \label{necesaria_potencial}

Sea $f : A \subset \mathbb{R}^n \to \mathbb{R}^n$ conservativo con $ f \in C^1$, entonces su matriz jacobiana $Df$ es contínua y simétrica.
...
Como $f$ es conservativo, existe $\phi : A \subset \mathbb{R}^n \to \mathbb{R}$, $ \phi \in C^2$ tal que $\nabla \phi = f$.  
...
Sea $ f : A \subset \mathbb{R}^n \to \mathbb{R}^n$ tal que cumple la condición necesaria para que exista función potencial (es decir tiene $Df$ contínua y simétrica).  Si además $ A$ es \emph{símplemente conexo}, entonces $f$ es conservativo.
\end{theorem}
...
\begin{example}
Dado $ A = \mathbb{R}^2 - \{(0,0)\}$.  Cláramente $A$ no es símplemente conexo.  Ahora definimos

\begin{eqnarray*} f : A &\to& \mathbb{R}^2 \\
f(x,y) &=& (2x, 2y) \end{eqnarray*}

Existe el siguiente $\phi \in C^2$

\begin{eqnarray*} \phi : A &\to& \mathbb{R} \\
...
Sea $f : R \subseteq \mathbb{R}^2 \to \mathbb{R} $, con $R = [a,b] \times [c,d]$ y $f$ acotada.
...
La definición para funciones de la forma $f : R \subseteq \mathbb{R}^3 \to \mathbb{R} $ es análoga.

\end{definition}

\begin{definition}
Sea $f : A \subseteq \mathbb{R}^2 \to \mathbb{R} $, con $A$ acotado y $f$ acotada.

Por ser $A$ acotado, el mismo se encuentra dentro de un rectángulo $R = [a,b] \times [c,d]$, es decir $A \subseteq R$.  

Definimos la función $h : R \to \mathbb{R}$ como
...
La definición para funciones de la forma $f : A \subseteq \mathbb{R}^3 \to \mathbb{R} $ es análoga.

\end{definition}

\begin{definition}

Sea $A \subseteq \mathbb{R}^2$ acotada.  Definimos su área como

$$ area(A) = \iint_A dxdy $$

Sea $H \subseteq \mathbb{R}^3$ acotada.  Definimos su volúmen como
...
\begin{definition}
Una \textbf{región elemental tipo I de $\mathbb{R}^2$} es un subconjunto $ R \subseteq \mathbb{R}^2$ de la forma

$$ R = \{(x,y) \in \mathbb{R}^2 : a \leq x \leq b, f_1(x) \leq y \leq f_2(x) \} $$
...
Una \textbf{región elemental de $\mathbb{R}^2$}, es una región elemental tipo I o bien tipo II.


Una \textbf{región elemental tipo I de $\mathbb{R}^3$} es un subconjunto $ R \subseteq \mathbb{R}^3$ de la forma

$$ R = \begin{cases} a \leq x \leq b \\ f_1(x) \leq y \leq f_2(x) \\ g_1(x,y) \leq z \leq g_2(x,y) \end{cases}$$

En total hay $3! = 6$ tipos de regiones elementales en $ \mathbb{R}^3$, que se corresponden con las permutaciones de las tres variables $x,y,z$ ya que básicamente consiste en ordenar las variables.
...
En $\mathbb{R}^2$, supongamos que tenemos la región $R$ definida por
...
\section{Teorema de Fubini en $\mathbb{R}^2$}

El siguiente teorema nos permite calcular ciertas integrales dobles realizando dos integrales simples iteradas.

\begin{theorem}
Sea $f : A \subset \mathbb{R}^2 \to \mathbb{R}$ contínua, con $ A$ región elemental tipo I, de la forma $A = \{(x,y) \in \mathbb{R}^2 : a \leq x \leq b, f(x) \leq y \leq g(x) \}$ entonces
...
Primero lo enunciamos en $\mathbb{R}^2$

\begin{theorem}

Sea $f : A \subset \mathbb{R}^2 \to \mathbb{R}$ contínua, y $g : U \subseteq \mathbb{R}^2 \to \mathbb{R}^2$ biyectiva, $g \in C^1$, y $(x,y) = g(u,v)$, y sea $ A = g(B)$, entonces

$$ \displaystyle \iint_{A=g(B)} f(x,y) dxdy = \iint_{B} f(g(u,v)) ||det(Dg(u,v))|| dudv $$

\end{theorem}

Ahora enunciamos la versión del teorema para $\mathbb{R}^3$

\begin{theorem}
Sea $f : A \subset \mathbb{R}^3 \to \mathbb{R}$ contínua, y $g : U \subseteq \mathbb{R}^3 \to \mathbb{R}^3$ biyectiva, $g \in C^1$, y $(x,y,z) = g(u,v,w)$, y sea $ A = g(B)$, entonces

$$ \displaystyle \iiint_{A=g(B)} f(x,y,z) dxdydz = \iiint_{B} f(g(u,v,w)) ||det(Dg(u,v,w))|| dudvdw $$

\end{theorem}

\section{Coordenadas cartesianas y polares en $\mathbb{R}^2$}

Son los cambios de variables más utilizados en $\mathbb{R}^2$

\begin{definition}
Las \textbf{coordenadas cartesianas} corresponde a la función identidad de $\mathbb{R}^2$

\begin{eqnarray*} g: \mathbb{R}^2 &\to& \mathbb{R}^2 \\
...
\begin{eqnarray*} g : [0, +\infty) \times [0, 2\pi) &\to& \mathbb{R}^2 \\
...
\section{Coordenadas cartesianas, cilíndricas y esféricas en $\mathbb{R}^3$}

Son los cambios de variables más utilizados en $\mathbb{R}^3$

\begin{definition}

Las \textbf{coordenadas cartesianas} corresponde a tomar la función identidad de $\mathbb{R}^3$

\begin{eqnarray*} g: \mathbb{R}^3 &\to& \mathbb{R}^3 \\
...
\begin{eqnarray*} g : [0, +\infty) \times [0, 2\pi) \times \mathbb{R} &\to& \mathbb{R}^3 \\
...
\begin{eqnarray*} g : [0, +\infty) \times [0, 2\pi) \times [0, \pi] &\to& \mathbb{R}^3 \\
...
\begin{definition}
Dada una superficie $\Sigma \in \mathbb{R}^3$ con parametrización $ g: B \subseteq \mathbb{R}^2 \to \mathbb{R}^3$, y dado un campo escalar $f : A \subseteq \mathbb{R}^3 \to \mathbb{R}$ contínuo, tal que $\Sigma \in A^{\circ}$, definimos el diferencial de superficie escalar como
...
Dada la misma superficie $\Sigma$ (orientable) con misma parametrización $g$, y dado el campo vectorial $f : A \subseteq \mathbb{R}^3 \to \mathbb{R}^3$ contínuo, tal que $\Sigma \in A^{\circ}$, definimos el diferencial de superficie vectorial como
...
Sea $f: A \subset \mathbb{R}^2 \to \mathbb{R}$, su conjunto gráfica consiste en el conjunto 

$$ \Sigma = \{(x,y,z) \in A \times \mathbb{R} : z = f(x,y)\}$$
...
Sea el campo escalar $ G: A \subset \mathbb{R}^3 \to \mathbb{R}$, $ G \in C^1$ y supongamos la superficie $\Sigma$ corresponde al conjunto de nivel 0 de $G$.  Es decir es la superficie de ecuación 
...
Sea $ C \subset \mathbb{R}^2$ una curva, y $A \subseteq \mathbb{R}^2$ una región plana.

Sea $ \delta: \mathbb{R}^2 \to \mathbb{R}$ contínua la densidad de masa.
...
Ahora en $\mathbb{R}^3$.  Veamos algunas fórmulas para el cálculo de masa, momentos y centro, sobre curvas en el espacio, regiones sólidas, y supercicies.

Sea $ C \subset \mathbb{R}^3$ una curva, $A \subseteq \mathbb{R}^3$ una región sólida, y $\Sigma \subseteq \mathbb{R}^3$ una superficie.

Sea $ \delta: \mathbb{R}^3 \to \mathbb{R}$ contínua la densidad de masa.
...
Sea $f : A \subset \mathbb{R}^2 \to \mathbb{R}^2$, $f \in C^1$,  $ f=(P,Q)$, entonces definimos el green de $f$ como

$$ green(f) = Q'_x - P'_y $$

El \textbf{operador nabla} corresponde a $ \nabla = \left( \frac{\partial}{\partial x}, \frac{\partial}{\partial y}, \frac{\partial}{\partial z} \right)$.

Sea $f : A \subset \mathbb{R}^3 \to \mathbb{R}^3$, $f \in C^1$, $ f=(P,Q,R)$, entonces definimos el rotor de $f$ como
...
Dada una región elemental $ A \subset \mathbb{R}^2$ con curva frontera $ C = \partial A$ regular o regular a trozos (automáticamente cerrada y simple, o sea de Jordan), y sea el campo vectorial $f : B \subset \mathbb{R}^2 \to \mathbb{R}^2$, $f \in C^1$, $f = (P,Q)$, y con $ A \subset B$, entonces
...
Dada una superficie (orientable) abierta $ \Sigma \subset \mathbb{R}^3$ y su curva borde $ C = \partial \Sigma$ regular/a trozos (automáticamente cerrada y simple), y sea $ f : A \subset \mathbb{R}^3 \to \mathbb{R}^3$, $f \in C^1$ con $ f = (P,Q,R)$, y con $ \Sigma \subset A$, entonces
...
Dada una región elemental del espacio $H \subset \mathbb{R}^3$, y su superficie frontera $ \Sigma = \partial H$ una superficie regular/a trozos (automáticamente cerrada y simple), y sea $ f : A \subset \mathbb{R}^3 \to \mathbb{R}^3$, $f \in C^1$, con $ f = (P,Q,R)$, y $ H \subset A$, entonces
...
\begin{definition}
Sea $A \subset \mathbb{R}^n$ un conjunto abierto.

Un campo vectorial $f: A \subset \mathbb{R}^3 \to \mathbb{R}^3$, $ f \in C^1$ se dice \textbf{irrotacional} si $ rot(f) = 0$

Un campo vectorial $f: A \subset \mathbb{R}^3 \to \mathbb{R}^3$, $ f \in C^1$ se dice \textbf{solenoidal} si $ div(f) = 0$

Un campo escalar $ f: A \subset \mathbb{R}^3 \to \mathbb{R}$, $ f \in C^1$ se dice \textbf{armónico} si $ div(grad(f)) = 0$

\end{definition}

\begin{theorem}
Sea el campo escalar $f: A \subset \mathbb{R}^3 \to \mathbb{R}$, $ f \in C^2$, entonces $ rot(grad(f)) = 0$, es decir los campos de gradientes son irrotacionales.

Sea el campo vectorial $f: A \subset \mathbb{R}^3 \to \mathbb{R}^3$ con $ f \in C^2$, entonces $div(rot(f)) = 0$, es decir los campos de rotores son solenoidales.
\end{theorem}
...
\begin{definition}
Una función $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^m$ se dice \textbf{homogénea de grado $k$} si 
...
con $P$ y $Q$ definidos sobre $A \subseteq \mathbb{R}^2$ símplemente conexo, y tal que $Q'_x - P'_y = 0$.
\end{definition}
...
$$\phi : A \subseteq \mathbb{R}^2 \to \mathbb{R}$$
...
con $P$ y $Q$ definidos sobre $A \subseteq \mathbb{R}^2$ símplemente conexo, pero tal que $Q'_x - P'_y \neq 0$.
...
La solución general de la homogénea asociada resulta ser un subespacio de dimensión dos del espacio vectorial de las funciones $C^2$ de $\mathbb{R}$ en $\mathbb{R}$.  Por lo tanto alcanza con encontrar una base de dicho subespacio, por lo tanto si $f_1, f_2$ son soluciones particulares y $ \{f_1, f_2\}$ es LI (linealmente independiente), entonces la solución general son sus combinaciones lineales, es decir

$$ y = C_1 f_1 + C_2 f_2 $$

\begin{definition}

Dadas $f_1, f_2 : A \subseteq \mathbb{R} \to \mathbb{R}$, $f_1, f_2 \in C^1$.  El \textbf{wronskiano} de $f_1$ y $f_2$ es el siguiente determinante
...
Dadas $f_1, f_2 : A \subseteq \mathbb{R} \to \mathbb{R}$, $f_1, f_2 \in C^1$
...
Dado un campo vectorial $f : A \in \mathbb{R}^n \to \mathbb{R}^n$, $f \in C^1$, una línea de campo es una curva regular $C \subseteq A$ que admite una parametrización $g : [a,b] \to \mathbb{R}^n$ tal que 
...
En $\mathbb{R}^2$, dada $f : A \subseteq \mathbb{R}^2 \to \mathbb{R}^2$.
...
\begin{theorem}
Si $f : A \subseteq \mathbb{R}^n \to \mathbb{R}^n$ es un campo conservativo, y $ \phi : A \subseteq \mathbb{R}^n \to \mathbb{R}$ es una función potencial de $f$, es decir $\nabla \phi = f$, entonces las líneas de campo de $f$ son ortogonales a los conjuntos de nivel de $\phi$.
\end{theorem}
...
En $\mathbb{R}^2$ tenemos el siguiente caso particular, que podemos demostrar de una forma más cartesiana.

\begin{theorem}
Sea $f : A \subseteq \mathbb{R}^2 \to \mathbb{R}^2$ un campo conservativo, y $\phi$ una función potencial de $f$, es decir tal que $\nabla \phi = f$.
